...
public class G10HW2 {
    ...
    // By using Farther-First Traversal algorithm, given a pointset S and a number of cluster k
    ...
    // extended for more generalized cases by using a Tuple3 which stores minimum distance from nearest cluster,
    // the nearest cluster and the coordinates of the point.
    public static ArrayList<Vector> kCenterMPD(ArrayList<Vector> s, int k)
    {

        ArrayList<Vector> centers = new ArrayList<>();
        ArrayList<Tuple2<Double, Vector>> distances = new ArrayList<>();
        ...
        // For every centroid, track the minimum distances between every point in S and every centroid
        // Then we can add the point with the maximum distance from the nearest centroid
        for(int i = 0; i < k-1; i++)
        {

            double max_dist = 0.0;
            int new_cluster = 0; // flag for tracking what cluster add next
            ...
            for(int j = 0; j < s.size(); j++)
            {
                ...
                // Only first cycle: fill the ArrayList with the distances from the random cluster
                if (i == 0) distances.add(new Tuple2<>(dist, s.get(j)));
                // Otherwise
                else {
                    ...
                    if(dist < distances.get(j)._1())
                    {
                        distances.set(j, new Tuple2<>(dist, distances.get(j)._2()));
                        if (max_dist < distances.get(j)._1()) {
                            max_dist = distances.get(j)._1();
                            new_cluster = j;
                        }

                    }
                    // Otherwise, check if the previous distance has the maximum distance
                    else if (max_dist < distances.get(j)._1())
                    {
                        max_dist = distances.get(j)._1();
                        new_cluster = j;
                    }
